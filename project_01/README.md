...
This folder solely contains sample audio files, this is what is loaded into the drum machine to actually produce sound. The files need to be named according to whatever is defined in the audio_manager.py, or else it won't work. 
...
The project folder is made to hold all the project files generated by/for the Mini Drum, it's technically not empty, but the file inside the projects folder is NOT necessary, think of it solely as a storage for the drummy os.
...
In my repository you will find a folder called tests. These test files aren't necessary to run this program, however, as I made each class/file, they were especially useful to make sure functions were working before I moved on to the next thing. I will not provide instructions for these files because they aren't guaranteed to work and they tested things I specifically wanted to know about, however, others might want to test other things. Also as a note, these test scripts were AI generated for the sake of time and finishing the project with the functionality that I desired. 
